...
const (
	...
	flarePackage    = protogen.GoImportPath("github.com/flare-rpc/flarego")
)
...
type serviceGenerateHelperInterface interface {
	...
	generateWrapperServerType(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, service *protogen.Service)
...
func (serviceGenerateHelper) generateWrapperServerType(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, service *protogen.Service) {
	serverType := service.GoName + "Server"

	// Server wrapper for net/rpc server interface
	g.P("// wrapper", serverType, " ", "is a wrapper for ", serverType, " that implements the interface of net/rpc Service")
	g.P("type wrapper", serverType, " struct {")
	g.P("svr ", serverType)
	g.P("}")
	g.P()
	for _, method := range service.Methods {
		g.P("func (w *wrapper", serverType, ") ", serverNetrpcSignature(g, method), "{")
		g.P("res, err := w.svr.", method.GoName, "(ctx, req)")
		g.P("if err != nil {")
		g.P("return err")
		g.P("}")
		g.P("*resp = *res")
...
func generateFile(gen *protogen.Plugin, file *protogen.File) *protogen.GeneratedFile {
	...
	filename := file.GeneratedFilenamePrefix + "_flare.pb.go"
	g := gen.NewGeneratedFile(filename, file.GoImportPath)
	g.P("// Code generated by protoc-gen-flarego. DO NOT EDIT.")
	g.P("// versions:")
	g.P("// - protoc-gen-flarego v", version)
...
func genService(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, service *protogen.Service) {
	...
	g.P("cc ", flarePackage.Ident("ClientConn"))
	g.P("}")
	g.P()
	...
	g.P("func New", clientName, " (cc ", flarePackage.Ident("ClientConn"), ") ", clientName, " {")
	...
	// generate wrapper struct for net/rpc
	helper.generateWrapperServerType(gen, file, g, service)
	...
	g.P("s.RegisterService(&", serviceDescVar, ",", "&wrapper"+serverType+"{srv})")
	g.P("}")
	g.P()
...
func clientSignature(g *protogen.GeneratedFile, method *protogen.Method) string {
	...
	s += ", opts ..." + g.QualifiedGoIdent(flarePackage.Ident("CallOption")) + ") ("
	if !method.Desc.IsStreamingClient() && !method.Desc.IsStreamingServer() {
...
func serverNetrpcSignature(g *protogen.GeneratedFile, method *protogen.Method) string {
	var reqArgs []string
	ret := "error"
	...
	reqArgs = append(reqArgs, "resp *"+g.QualifiedGoIdent(method.Output.GoIdent))
